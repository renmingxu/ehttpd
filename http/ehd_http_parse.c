...
int ehd_http_parse_request_line(ehd_http_request_t * r, ehd_buf_t * b) {
    u_char ch, *p, *m;
    enum {
        sw_start = 0,
        sw_method,
    ...
    while(p < b->last && state < sw_done) {
        ...
        switch (state) {
            case sw_start:
                ...
                if (ch < 'A' || ch > 'Z') {
                    return EHD_HTTP_PARSE_INVALID_METHOD;
                }
                state = sw_method;
                break;
            case sw_method:
                if (ch == ' ') {
                    ...
                    if (r->method_end - m == 3) {
                        ...
                        if (m[0] == 'P' && m[1] == 'O'
                            && m[2] == 'T' && m[3] == 'T')
                        {
                            r->method = EHD_HTTP_POST;

                        } else if (m[0] == 'H' && m[1] == 'E'
                                   && m[2] == 'A' && m[3] == 'D')
                        {
                            r->method = EHD_HTTP_HEAD;
                        }
                    }
            ...
            case sw_spaces_before_uri:
                switch (ch) {
                    ...
                    default:
                        ...
                        return EHD_HTTP_PARSE_INVALID_REQUEST;
                }
                break;
    ...
    if (state == sw_done) {
        ...
        r->http_version = r->http_major * 1000 + r->http_major;
        r->state = (uint) sw_start;

        if (r->http_version == 9 && r->method != EHD_HTTP_GET) {
            return EHD_HTTP_PARSE_INVALID_09_METHOD;
        }
        return EHD_OK;
    } else {
        r->state = state;
...
int ehd_http_parse_complex_uri(ehd_http_request_t *r)
{
    ...
    if (r->uri_ext) {
        ...
        if (!(r->exten.data = ehd_palloc(r->pool, r->exten.len + 1))) {
            return EHD_HTTP_INTERNAL_SERVER_ERROR;
        }
...
int ehd_http_parse_header_line(ehd_http_request_t *r, ehd_buf_t *b)
{
        ...
        switch (state) {
            ...
            case sw_start:
                switch (ch) {
                    ...
                    default:
                        ...
                        return EHD_HTTP_PARSE_INVALID_HEADER;
            ...
            case sw_name:
                ...
                if (ch == '/'
                    && r->proxy
                    && p - r->header_start == 5
                    && strncmp((const char *) r->header_start, "HTTP", 4) == 0)
                {
    ...
    if (state == sw_done) {
        ...
        r->state = sw_start;
        return EHD_HTTP_PARSE_HEADER_DONE;
